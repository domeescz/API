...
#Úprava existujícího zákazníka metodu PUT, popř. vytvoření nového zákazníka, není li v db
@app.route('/customer/<string:username>',methods=['PUT'])
def updated_customer(username):
    request_data = request.get_json()
    update_customer = {
                "email": request_data['email'],
                "name": request_data['name'],
    ...
    for customer in customers:
        if customer['username'] == username:
            ...
            return jsonify(update_customer), 200
...
def delete_customer(username):
    for customer in customers:
        ...
        return jsonify('The user has not been found in the database.'),404
# Spuštění serveru
